...
/* ---------------------------------------------------------
   Design tokens
   bg        #14151A   near-black, faint blue-cold (editing suite)
   panel     #1D1E24
   panel-2   #24252C
   line      #33343C
   text      #ECEDEF
   muted     #8B8D97
   amber     #FFB020   record / active
   teal      #35D0A8   confirm / stop
   red       #FF5C5C   discard
   Type: JetBrains Mono for all timecodes/numerals, Inter for everything else.
--------------------------------------------------------- */

const TAG_PALETTE = [
  "#FFB020", "#35D0A8", "#7C9CFF", "#FF8A65", "#C792EA", "#4FD1FF", "#FFD166", "#EF8FA1",
];
...
function Pill({ active, onClick, children, color, onRemove }) {
  return (
    <button
      onClick={onClick}
      style={{
        borderColor: active ? (color || "#FFB020") : "#33343C",
        background: active ? `${color || "#FFB020"}22` : "transparent",
        color: active ? (color || "#FFB020") : "#8B8D97",
      }}
      className="group flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[13px] font-medium transition-colors whitespace-nowrap"
    >
      {color && (
...
function Avatar({ name, size = 28 }) {
  ...
  return (
    <div
      style={{ width: size, height: size, background: `${color}26`, color, border: `1px solid ${color}55` }}
      className="flex items-center justify-center rounded-full font-mono text-[12px] font-semibold shrink-0"
    >
      {initial}
    </div>
  );
  ...
  if (!loaded) {
    return (
      <div className="min-h-screen bg-[#14151A] flex items-center justify-center">
        <div className="font-mono text-[#8B8D97] text-sm animate-pulse">loading log…</div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-[#14151A] text-[#ECEDEF]" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600;700&display=swap');
        .font-mono { font-family: 'JetBrains Mono', ui-monospace, monospace; }
        ::-webkit-scrollbar { width: 8px; height: 8px; }
        ::-webkit-scrollbar-thumb { background: #33343C; border-radius: 4px; }
      `}</style>

      {saveError && (
        <div className="bg-[#FF5C5C22] border-b border-[#FF5C5C55] text-[#FF9A9A] text-[12px] text-center py-1.5 px-4">
      ...
      <main className="max-w-3xl mx-auto px-4 pb-28">
        ...
        <div className="flex items-center gap-1 mt-8 mb-4 border-b border-[#24252C]">
  ...
  return (
    <header className="max-w-3xl mx-auto px-4 pt-8 pb-2">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <div className="flex items-center gap-2 text-[#FFB020] font-mono text-[11px] tracking-[0.2em] uppercase mb-1">
            <span className="h-1.5 w-1.5 rounded-full bg-[#FFB020]" />
            Lead Time Log
          </div>
          <h1 className="text-[22px] font-semibold tracking-tight">Where did the day go</h1>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {addingUser ? (
            <form
              ...
              className="flex items-center gap-1 rounded-full border border-[#33343C] pl-3 pr-1 py-1 bg-[#1D1E24]"
            >
              <input
                autoFocus
                value={newUserName}
                ...
                placeholder="Name"
                className="bg-transparent outline-none text-[13px] w-20 placeholder:text-[#5A5B63]"
              />
              <button type="submit" className="rounded-full bg-[#FFB020] text-[#14151A] p-1">
                <Check size={12} />
              </button>
            </form>
          ) : (
            <button
              ...
              className="flex items-center gap-1 rounded-full border border-dashed border-[#33343C] text-[#8B8D97] px-3 py-1.5 text-[13px] hover:border-[#FFB020] hover:text-[#FFB020] transition-colors"
            >
              <Plus size={13} /> Person
            </button>
          )}
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 px-3 py-2.5 text-[13px] font-medium border-b-2 -mb-px transition-colors ${
        active ? "border-[#FFB020] text-[#ECEDEF]" : "border-transparent text-[#8B8D97] hover:text-[#ECEDEF]"
      }`}
    >
      {icon} {label}
    </button>
...
function TimerCard({ currentUser, running, now, onStart, onStop, onManual }) {
  ...
  return (
    <div className="mt-6 rounded-2xl border border-[#24252C] bg-[#1D1E24] p-5 flex items-center justify-between gap-4 flex-wrap">
      <div className="flex items-center gap-4">
        <button
          onClick={running ? onStop : onStart}
          disabled={!currentUser}
          style={{
            background: running ? "#FF5C5C" : "#FFB020",
            boxShadow: running ? "0 0 0 6px #FF5C5C1A" : "0 0 0 6px #FFB0201A",
          }}
          className="flex items-center justify-center h-14 w-14 rounded-full text-[#14151A] disabled:opacity-30 disabled:shadow-none transition-all shrink-0"
        >
          {running ? <Square size={20} fill="#14151A" /> : <Play size={22} fill="#14151A" className="ml-0.5" />}
        </button>
        <div>
          <div className="font-mono text-[28px] font-semibold tabular-nums leading-none">
            {running ? fmtElapsed(elapsed) : "00:00"}
          </div>
          <div className="text-[13px] text-[#8B8D97] mt-1">
            {!currentUser
      ...
      <button
        onClick={onManual}
        disabled={!currentUser}
        className="text-[13px] font-medium text-[#8B8D97] hover:text-[#ECEDEF] disabled:opacity-30 border border-[#33343C] rounded-full px-4 py-2 transition-colors"
      >
        Log manually
      </button>
    </div>
...
function PendingSheet({ pending, setPending, tags, tagQuery, setTagQuery, onToggleTag, onAddTag, onConfirm, onDiscard, currentUser, manual }) {
  ...
  return (
    <div className="fixed inset-0 z-20 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm px-0 sm:px-4">
      <div className="w-full sm:max-w-md bg-[#1D1E24] border-t sm:border border-[#2C2D35] rounded-t-2xl sm:rounded-2xl p-5 max-h-[88vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            <Avatar name={currentUser?.name} size={22} />
            <span className="text-[13px] text-[#8B8D97]">{currentUser?.name}</span>
          </div>
          <button onClick={onDiscard} className="text-[#8B8D97] hover:text-[#ECEDEF]">
            <X size={18} />
          </button>
        </div>

        <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center gap-1.5 font-mono text-[13px] text-[#8B8D97]">
            <Clock size={13} />
            {manual ? (
              <>
                <input
                  ...
                  className="bg-[#24252C] border border-[#33343C] rounded px-1.5 py-0.5 text-[#ECEDEF]"
                />
                <span>–</span>
                <input
                  type="time"
          ...
          <span className="text-[13px] font-mono text-[#FFB020] font-semibold">{fmtDuration(durationMs)}</span>
        </div>
        ...
        <input
          ...
          className="w-full bg-[#24252C] border border-[#33343C] rounded-lg px-3 py-2.5 text-[14px] outline-none focus:border-[#FFB020] placeholder:text-[#5A5B63] mb-4"
        />

        <div className="mb-2 text-[12px] uppercase tracking-wide text-[#8B8D97]">Tags</div>
        ...
        {pending.tagIds.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-2">
            {pending.tagIds.map((id) => {
              ...
              if (!t) return null;
              return (
                <Pill key={id} active color={tagColor(t.name)} onClick={() => onToggleTag(id)} onRemove={() => onToggleTag(id)}>
                  {t.name}
                </Pill>
              );
            })}
        ...
        <div className="flex items-center gap-1.5 mb-2">
          <input
            ...
            className="flex-1 bg-[#24252C] border border-[#33343C] rounded-lg px-3 py-2 text-[13px] outline-none focus:border-[#FFB020] placeholder:text-[#5A5B63]"
          />
          {tagQuery.trim() && !exactExists && (
            <button onClick={onAddTag} className="shrink-0 rounded-lg bg-[#35D0A822] text-[#35D0A8] px-2.5 py-2 text-[12px] font-medium border border-[#35D0A855]">
        ...
        {filteredTags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4 max-h-28 overflow-y-auto">
            {filteredTags.filter((t) => !pending.tagIds.includes(t.id)).map((t) => (
              <Pill key={t.id} color={tagColor(t.name)} onClick={() => onToggleTag(t.id)}>
                {t.name}
              </Pill>
            ))}
          </div>
        ...
        <div className="flex items-center gap-2 mt-5">
          <button
            onClick={onDiscard}
            className="flex-1 py-2.5 rounded-lg border border-[#33343C] text-[#8B8D97] text-[13px] font-medium hover:text-[#ECEDEF] transition-colors"
          >
            Discard
          </button>
          <button
            onClick={onConfirm}
            className="flex-1 py-2.5 rounded-lg bg-[#35D0A8] text-[#0F1512] text-[13px] font-semibold hover:brightness-110 transition-all"
          >
            Save entry
          </button>
        </div>
...
function LogView({ grouped, users, tags, onDelete }) {
  if (grouped.length === 0) {
    return (
      <div className="text-center py-16 text-[#5A5B63]">
        ...
        return (
          <div key={day.key}>
            <div className="flex items-baseline justify-between mb-2.5">
              <div className="text-[13px] font-semibold text-[#ECEDEF]">{day.label}</div>
              <div className="text-[12px] font-mono text-[#5A5B63]">{fmtDuration(totalMs)} logged</div>
            </div>
            <div className="space-y-1.5">
              {day.items.map((e) => {
                ...
                return (
                  <div
                    key={e.id}
                    className="group flex items-center gap-3 rounded-xl border border-[#24252C] bg-[#1A1B20] px-3.5 py-2.5 hover:border-[#33343C] transition-colors"
                  >
                    <Avatar name={user?.name} />
                    <div className="min-w-0 flex-1">
                      <div className="text-[14px] truncate">{e.description}</div>
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <span className="font-mono text-[11px] text-[#5A5B63]">
                        ...
                        {e.tagIds.map((id) => {
                          ...
                          if (!t) return null;
                          return (
                            <span key={id} style={{ color: tagColor(t.name) }} className="text-[11px] font-medium">
                              #{t.name}
                            </span>
                          );
                        })}
                    ...
                    <div className="text-[12px] font-mono text-[#8B8D97] shrink-0">{fmtDuration(dur)}</div>
                    <button
                      onClick={() => onDelete(e.id)}
                      className="opacity-0 group-hover:opacity-100 text-[#5A5B63] hover:text-[#FF5C5C] transition-opacity shrink-0"
                    >
                      <X size={14} />
                    </button>
                  </div>
...
function Dashboard({ entries, users, tags }) {
  ...
  const byTag = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([name, rec]) => ({
        ...
        color: name === "Untagged" ? "#33343C" : tagColor(name),
      }))
  ...
  return (
    <div>
          ...
          {[7, 14, 30].map((d) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-full text-[12px] font-medium border transition-colors ${
                range === d ? "border-[#FFB020] text-[#FFB020] bg-[#FFB02015]" : "border-[#33343C] text-[#8B8D97]"
              }`}
            >
              {d}d
            </button>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-[#5A5B63] text-[14px]">Nothing in this window.</div>
      ) : selected ? (
        <TagDetail
          tag={selected}
        ...
        <>
          ...
          {insights.length > 0 && (
            <div className="mb-8 rounded-xl border border-[#FFB02033] bg-[#FFB02010] p-4">
              <div className="text-[12px] uppercase tracking-wide text-[#FFB020] mb-2.5 font-semibold">Where the time's going</div>
              <ul className="space-y-1.5">
                {insights.map((ins, i) => (
                  <li key={i} className="text-[13px] text-[#D8D9DE] leading-snug flex gap-2">
          ...
          <div className="mb-2 flex items-center justify-between">
            <div className="text-[12px] uppercase tracking-wide text-[#8B8D97]">Time by tag</div>
            <div className="text-[11px] text-[#5A5B63]">tap a tag for detail</div>
          </div>
          <div className="space-y-1.5 mb-8">
            {byTag.map((t) => (
              <button
                ...
                className="w-full flex items-center gap-3 rounded-xl border border-[#24252C] bg-[#1A1B20] px-3.5 py-2.5 hover:border-[#33343C] transition-colors text-left"
              >
                ...
                <span className="text-[11px] text-[#5A5B63] font-mono">{t.count} entries · avg {t.avgMins}m</span>
                <span className="font-mono text-[13px] font-semibold w-14 text-right">{t.hours}h</span>
                <div className="w-20 h-1.5 rounded-full bg-[#24252C] overflow-hidden shrink-0 hidden sm:block">
          ...
          <div className="grid sm:grid-cols-2 gap-8">
            <div>
              <div className="text-[12px] uppercase tracking-wide text-[#8B8D97] mb-3">Share of time</div>
              <ResponsiveContainer width="100%" height={220}>
                <PieChart>
                  <Pie data={byTag} dataKey="hours" nameKey="name" innerRadius={50} outerRadius={80} paddingAngle={2}>
                    {byTag.map((t, i) => <Cell key={i} fill={t.color} />)}
                  </Pie>
                  <Tooltip
                    contentStyle={{ background: "#1D1E24", border: "1px solid #33343C", borderRadius: 8, fontSize: 12 }}
            ...
            <div>
              <div className="text-[12px] uppercase tracking-wide text-[#8B8D97] mb-3">By person</div>
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={byPerson}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#24252C" vertical={false} />
                  <XAxis dataKey="name" tick={{ fill: "#8B8D97", fontSize: 11 }} axisLine={{ stroke: "#33343C" }} tickLine={false} />
                  <YAxis tick={{ fill: "#8B8D97", fontSize: 11 }} axisLine={false} tickLine={false} />
                  <Tooltip
                    contentStyle={{ background: "#1D1E24", border: "1px solid #33343C", borderRadius: 8, fontSize: 12 }}
                    formatter={(v) => [`${v}h`, "hours"]}
                  />
                  <Bar dataKey="hours" fill="#FFB020" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
...
function TagDetail({ tag, entries, users, onBack }) {
  ...
  return (
    <div>
      <button onClick={onBack} className="flex items-center gap-1.5 text-[13px] text-[#8B8D97] hover:text-[#ECEDEF] mb-4">
      ...
      <div className="grid sm:grid-cols-2 gap-8 mb-8">
        <div>
          <div className="text-[12px] uppercase tracking-wide text-[#8B8D97] mb-3">By day</div>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={byDay}>
              <CartesianGrid strokeDasharray="3 3" stroke="#24252C" vertical={false} />
              <XAxis dataKey="label" tick={{ fill: "#8B8D97", fontSize: 11 }} axisLine={{ stroke: "#33343C" }} tickLine={false} />
              <YAxis tick={{ fill: "#8B8D97", fontSize: 11 }} axisLine={false} tickLine={false} />
              <Tooltip
                contentStyle={{ background: "#1D1E24", border: "1px solid #33343C", borderRadius: 8, fontSize: 12 }}
                formatter={(v) => [`${v}h`, "hours"]}
              />
              <Bar dataKey="hours" fill={tag.color} radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={byPerson} layout="vertical" margin={{ left: 0, right: 20 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#24252C" horizontal={false} />
              <XAxis type="number" tick={{ fill: "#8B8D97", fontSize: 11 }} axisLine={{ stroke: "#33343C" }} tickLine={false} />
              <YAxis type="category" dataKey="name" width={70} tick={{ fill: "#ECEDEF", fontSize: 12 }} axisLine={false} tickLine={false} />
              <Tooltip
                contentStyle={{ background: "#1D1E24", border: "1px solid #33343C", borderRadius: 8, fontSize: 12 }}
                formatter={(v) => [`${v}h`, "hours"]}
              />
              <Bar dataKey="hours" fill={tag.color} radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>

      <div className="text-[12px] uppercase tracking-wide text-[#8B8D97] mb-3">Entries</div>
      <div className="space-y-1.5">
        {sorted.map((e) => {
          const user = users.find((u) => u.id === e.userId);
          const dur = new Date(e.end) - new Date(e.start);
          return (
            <div key={e.id} className="flex items-center gap-3 rounded-xl border border-[#24252C] bg-[#1A1B20] px-3.5 py-2.5">
              <Avatar name={user?.name} />
              <div className="min-w-0 flex-1">
                <div className="text-[14px] truncate">{e.description}</div>
                <div className="font-mono text-[11px] text-[#5A5B63] mt-0.5">{fmtTimeShort(e.start)}–{fmtTimeShort(e.end)}</div>
              </div>
              <div className="text-[12px] font-mono text-[#8B8D97] shrink-0">{fmtDuration(dur)}</div>
            </div>
          );
...
function Stat({ label, value }) {
  return (
    <div className="rounded-xl border border-[#24252C] bg-[#1A1B20] px-4 py-3">
      <div className="font-mono text-[20px] font-semibold">{value}</div>
      <div className="text-[11px] text-[#8B8D97] mt-0.5">{label}</div>
    </div>
  );
}
